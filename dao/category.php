 ...
 * truy van tat ca 
 */
function category_select_by_discount()
{
    $sql = "SELECT * FROM tbl_categories cate JOIN tbl_products pro ON cate.category_id = pro.category_id WHERE pro.discount > 0 GROUP BY cate.category_id HAVING COUNT(pro.product_id)>0 order by cate.category_id";
    return pdo_query($sql);
}
/**
 * truy van 8 cai
 */
function category_select_limit_banner()
{
    $sql = "SELECT * FROM tbl_categories order by category_id LIMIT 0,8";
    return pdo_query($sql);
}
